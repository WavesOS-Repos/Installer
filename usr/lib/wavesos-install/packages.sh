...
install_desktop_environment() {
    ...
    log "Installing desktop environment(s) based on selection: $selection_summary"
    
    # Always install WavesOS packages (compulsory)
    install_wavesos_packages
    
    # Install specific packages based on selection(s)
    for de in "${SELECTED_DE[@]}"; do
        case "$de" in
            ...
            "cosmic")
                ...
                ;;
            *)
                error "Invalid desktop environment selection: $de"
                ;;
        esac
    done
